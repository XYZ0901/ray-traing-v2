...
color ray_color(const ray &r, const color &background, const hittable &world, int depth) {
    ...
    // If the ray this nothing, return the background color.
...
void count_pixel(const int &i, const int &j, const int &iw, const int &ih,
                 const camera &cam, const hittable_list &world, int max_depth);
...
int main() {
    ...
    for (int j = image_height - 1; j >= 0; --j) {
        std::cerr << "\rScanlines remaining: " << j << ' '
                  << "Scanlines percent: " << int(double(image_height - j) / double(image_height) * 100) << "% "
                  << float(clock() - start) / CLOCKS_PER_SEC << "s " << std::flush;
...
//        }
        ...
        cs.clear();
        cs.resize(image_width);
        for (int i = 0; i < image_width; ++i) {
            ts.emplace_back(count_pixel, i, j, image_width, image_height, cam, world, max_depth);
        }
        for (auto &t: ts) {
            t.join();
        }
...
void count_pixel(const int &i, const int &j, const int &iw, const int &ih,
    ...
    color pixel_color(0, 0, 0);
    for (int s = 0; s < 10; ++s) {
        auto u = (i + random_double()) / (iw - 1);
        auto v = (j + random_double()) / (ih - 1);
        ray r = cam.get_ray(u, v);
        pixel_color += ray_color(r, {0, 0, 0}, world, max_depth);
    }
    cs[i] = pixel_color;
}
...
hittable_list random_scene() {
    ...
    auto checker = make_shared<checker_texture>(color(.2, .3, .1),
                                                color(.9, .9, .9));
...
hittable_list final_scene() {
    ...
    auto ground = make_shared<lambertian>(color(.48, .83, .53));
    ...
    auto moving_sphere_material = make_shared<lambertian>(color(.7, .3, .1));
    ...
    objects.add(make_shared<sphere>(point3(0, 150, 145), 50,
                                    make_shared<metal>(color(.8, .8, .9), 1.)));
